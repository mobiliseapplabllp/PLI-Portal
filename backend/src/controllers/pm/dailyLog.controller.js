...
const getLogs = async (req, res, next) => {
  try {
    const { logs, pagination } = await dailyLogService.getLogs(req.params.id, req.query, req.user);
    sendPaginated(res, logs, pagination);
  }
  catch (e) { next(e); }
};
const getLogById = async (req, res, next) => {
  try { sendSuccess(res, await dailyLogService.getLogById(req.params.id, req.params.logId, req.user)); }
  catch (e) { next(e); }
};
...
const getTodayLog = async (req, res, next) => {
  try { sendSuccess(res, await dailyLogService.getTodayLog(req.params.id, req.user)); }
  catch (e) { next(e); }
};
